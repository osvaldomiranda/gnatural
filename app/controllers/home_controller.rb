class HomeController < ApplicationController
  ...
  def index
    ...
    vta_producto_mes = "SELECT  name_template, sum(price_subtotal_incl) FROM tablon_mes WHERE rut_centro='76657776-8' GROUP BY name_template "
  #  array = ActiveRecord::Base.establish_connection(conn2).connection.execute(sql).to_a
    ...
    vta_diaria_total = "SELECT to_char(date_trunc('day', create_date_order), 'YYYY-MM-DD') AS date , sum(price_subtotal_incl) FROM tablon_mes WHERE rut_centro='76657776-8' GROUP BY date ORDER BY date "
  #  array = ActiveRecord::Base.establish_connection(conn2).connection.execute(sql).to_a
    ...
    vta_diaria_item = "SELECT name_template, to_char(date_trunc('day', create_date_order), 'YYYY-MM-DD') AS date , sum(price_subtotal_incl) FROM tablon_mes WHERE rut_centro='76657776-8' GROUP BY name_template,date ORDER BY name_template "
    ...
    vta_anual = "SELECT  to_char(date_trunc('month', create_date_order), 'YYYY-MM-DD') AS date , sum(price_subtotal_incl) FROM tablon2_anual WHERE rut_centro='76657776-8' AND create_date_order > date_trunc('month', CURRENT_DATE) - INTERVAL '1 year' GROUP BY date ORDER BY date"
